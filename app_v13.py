...
def inject_css():
    ...
    [data-testid="stSidebar"] > div {{ width:220px !important; }}
...
def load_all_data(_unused=None):
    dfs = []
    fuentes = [
        (CARPETA_ENTRANTE,"ENTRANTE",
         ["CARRIER_ORIGEN","CARRIER_DESTINO","CARRIER"],
         ["RUTA_ORIGEN","RUTA_DESTINO","RUTA"]),
        (CARPETA_SALIENTE,"SALIENTE",
         ["CARRIER_DESTINO","CARRIER_ORIGEN","CARRIER"],
         ["RUTA_DESTINO","RUTA_ORIGEN","RUTA"]),
    ]
    for carpeta,tipo,carrier_opts,ruta_opts in fuentes:
        if not os.path.isdir(carpeta):
            st.warning(f"Carpeta no encontrada: {carpeta}"); continue
        sufijo  = "entrante" if tipo=="ENTRANTE" else "saliente"
        latest  = os.path.join(carpeta,f"maestro_{sufijo}_LATEST.csv")
        archivos= [latest] if os.path.exists(latest) else glob(os.path.join(carpeta,"*.csv"))
        if not archivos:
            st.warning(f"Sin CSVs en: {carpeta}"); continue
        for archivo in archivos:
            try:
                ...
                if not col_carrier or not col_ruta:
                    st.warning(f"Columnas no reconocidas: {cols}"); continue
                ...
                dfs.append(df[["FECHA","TIPO","CARRIER","RUTA","ASR",
                                "CALLS_TOTAL","CALLS_OK","MINUTOS","AVG_CALL"]])
            except Exception as e:
                st.warning(f"Error leyendo {os.path.basename(archivo)}: {e}")
